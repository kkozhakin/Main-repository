...
namespace ImageEdit
{
    class FinderDomainColor
    {
        public static Color FindDomainColor(Bitmap bmp)
        {
            const int quant = 20;
            using (var gr = Graphics.FromImage(bmp))
            {
                //gr.CopyFromScreen(0, 0, 0, 0, scrSize);

                var freq = new Dictionary<Color, int>();
                var maxColor = Color.Empty;
                var maxFreq = 0;

                using (var small = (Bitmap)bmp.GetThumbnailImage(200, 200, null, IntPtr.Zero))
                using (var wr = new ImageWrapper(small))
                    foreach (var p in wr)
                    {
                        var c = wr[p];
                        c = Color.FromArgb(c.R / quant, c.G / quant, c.B / quant);
                        var count = 0;
                        freq.TryGetValue(c, out count);
                        count++;
                        freq[c] = count;
                ...
                return Color.FromArgb(maxColor.R * quant, maxColor.G * quant, maxColor.B * quant);
            }
