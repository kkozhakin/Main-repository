...
#define TEST_ITERATOR
    ...
    class MyIterator : public std::bidirectional_iterator_tag // You can rename it and extend it from std::iterator if you need for some reason.
    {


    public:
        explicit MyIterator(Node* cur, Node* pre): _cur(cur), _preTail(pre) {}
    ...
    public:
        explicit Iterator(Node* cur, Node* pre) : MyIterator(cur, pre) {}
        ...
        Iterator& operator--()
        {
            if (!this->_cur && !this->_preTail)
                throw std::logic_error("");
            if (this->_cur == nullptr)
                this->_cur = this->_preTail;
            else
            {
                this->_preTail = this->_cur;
    ...
    class Reverse_Iterator: public MyIterator
    {

    public:
        explicit Reverse_Iterator(Node* cur, Node* pre) : MyIterator(cur, pre) {}
...
public:
    ...
    */
    ...
    {
        if (_tail)
            return Iterator(_tail->_next, _tail);
    ...
    {
        return Reverse_Iterator(_tail, nullptr);
    }

    Reverse_Iterator rend()
    {
        if (_head)
            return Reverse_Iterator(_head->_prev, _head);
