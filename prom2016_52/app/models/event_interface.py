...
def add_mark(d, _user):
    ...
    db.session.add(Event(d['name'], d['desc'], str(round(float(d['longitude']), 3)),
                         str(round(float(d['latitude']), 3)), d['city'], 'flags', owner.id,
                         d['adr']))
    db.session.commit()
...
def new_search(_data):
    ...
    if 'city' in search_dict.keys():
        base_search_dict.update(name=search_dict['city'])
...
def change_event_info(_id, new_name, new_city, new_text):
    event = Event.query.filter_by(id=_id).update(dict(city=new_city, name=new_name, text=new_text))
    db.session.commit()
